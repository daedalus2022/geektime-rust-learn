...
/// 提供 Strorage iterator， 这样 trait 的实现者只需要
/// 把他们的 iterator 提供给 StorageIter， 然后他们保证
...
mod tests {
    ...
    fn test_basi_interface(store: impl Storage) {
        // 第一次 set 会创建 table,  插入 key 并返回 None (之前没有值)
        ...
        // constains 存在的 key 返回 true， 否则 false
    ...
    fn test_get_all(store: impl Storage) {
        let _ = store.set("t2", "k1".into(), "v1".into());
        let _ = store.set("t2", "k2".into(), "v2".into());
